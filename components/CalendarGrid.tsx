import { format, startOfMonth, endOfMonth, eachDayOfInterval, getDay, isToday, isWeekend, isSameDay } from 'date-fns'
...
/**
 * Props interface for CalendarGrid component
 */
...
/**
 * CalendarGrid component - displays a monthly calendar with date selection
 */
export default function CalendarGrid({
  ...
  // Get start and end of current month for proper grid calculation
  ...
  // This determines how many empty cells to show before the 1st
  const startDayOfWeek = getDay(monthStart)
  
  // Create array of empty cells for days before month starts
  // This ensures the calendar grid aligns properly with weekdays
  const emptyCells = Array(startDayOfWeek).fill(null)
  
  // Combine empty cells and month days for complete grid
  ...
  const getRangeClass = (date: Date, index: number): string => {
   ...
   * @returns Holiday name or undefined if no holiday
   */
